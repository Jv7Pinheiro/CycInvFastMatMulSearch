function [df, f] = CI_Gradient_FunctionValue(Mat, X)
    ...
    %% Get Gradients
    dS = 3*(S*(SS.*SS) + U*(VS.*WS) + V*(US.*WS) + W*(US.*VS)) - (mttkrp(X, {S S S}, 1) + mttkrp(X, {S S S}, 2) + mttkrp(X, {S S S}, 3));
    dU = 3*(S*(SV.*SW) + U*(VV.*WW) + V*(WV.*UW) + W*(UV.*VW)) - (mttkrp(X, {U W V}, 1) + mttkrp(X, {V U W}, 2) + mttkrp(X, {W V U}, 3));
    dV = 3*(S*(SU.*SW) + U*(WU.*VW) + V*(UU.*WW) + W*(VU.*UW)) - (mttkrp(X, {V U W}, 1) + mttkrp(X, {W V U}, 2) + mttkrp(X, {U W V}, 3));
    dW = 3*(S*(SU.*SV) + U*(VU.*WV) + V*(WU.*UV) + W*(UU.*VV)) - (mttkrp(X, {W V U}, 1) + mttkrp(X, {U W V}, 2) + mttkrp(X, {V U W}, 3));
